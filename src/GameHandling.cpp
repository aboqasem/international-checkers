...
void Game::start(char &PLAYER, Board &BOARD) //Handling and applying rules to the game.
{
    ...
    cin.tie(NULL);
    string y1x1, y2x2, error, previousdirection;
    int x1 = -1, y1 = -1, x2 = -1, y2 = -1, xnow = -1, ynow = -1, previousx = 10, previousy = 10, currentx = 10, currenty = 10, onemove = 1;
    char OTHERPLAYER;
    vector<pair<int, int>> coordinatestocapture;
    Rules *Ordinary = new OrdinaryRules;
    Rules *Crown = new CrownRules;
    do
    {
        previousdirection = "";
        cout << "----------------------------------------------\n"
             ...
             << "----------------------------------------------\n";
        BOARD.printBoard();
        if (Rules::endgame(BOARD))
        {
            break;
        };
        PLAYER % 2 != 0 ? PLAYER = 'a' : PLAYER = 'b';
        PLAYER == 'a' ? OTHERPLAYER = 'b' : OTHERPLAYER = 'a';
        cout << endl
             << error << endl;
        error = "";
        cout << "\n# Player [" << PLAYER << "] turn ->";
        cin >> y1x1;
        for_each(y1x1.begin(), y1x1.end(), [](char &c) { c = tolower(c); });
        if (y1x1[0] != '0' && y1x1[0] != 's' && y1x1[0] != 'l')
        {
            cin >> y2x2;
            for_each(y2x2.begin(), y2x2.end(), [](char &c) { c = tolower(c); });
        }
        /*-------------------------------------------------------------------------------------------------*/
        if (y1x1[0] != '0' && y1x1[0] != 's' && y1x1[0] != 'l')
        {
            ...
            else if (Rules::basicRules(x1, y1, x2, y2, PLAYER, BOARD, error))
            {
                if (BOARD.getBoard(x1, y1) == PLAYER && Ordinary->legalMove(x1, y1, x2, y2, OTHERPLAYER, BOARD, error, onemove, currentx, currenty))
                {
                    ...
                    else
                    {
                        ...
                        if (Rules::becomeCrown(y2, PLAYER))
                        {
